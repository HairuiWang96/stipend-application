                ...
                <div className='grid grid-cols-1 md:grid-cols-2 gap-4 mt-4'>
                    ...
                    <div>
                        ...
                        <div className='relative'>
                            <input id='ssn' type='text' placeholder='XXX-XX-XXXX' autoComplete='off' {...register('ssn')} className={`w-full border rounded px-3 py-2 pr-10 ${!showSSN ? 'text-security-disc' : ''}`} style={!showSSN ? { WebkitTextSecurity: 'disc' } as React.CSSProperties : undefined} />
                            <button type='button' onClick={() => setShowSSN(!showSSN)} className='absolute right-2 top-1/2 -translate-y-1/2 text-gray-500 hover:text-gray-700 p-1' aria-label={showSSN ? 'Hide SSN' : 'Show SSN'}>
                                {/* When hidden: show open eye (click to see). When visible: show closed eye (click to hide) */}
                                {showSSN ? (
